...
def train(config, dict_source_dataloader, indexed_T_train_loader,T_train_loader,T_test_loader, model, optimizer):  # 模型训练算法
    ...
    return train_loss,rmse_loss,mmd_loss,  features_s.reshape(-1, 256), features_t.reshape(-1, 256), labels_s.reshape(-1, 1),dict_pred_S,dict_real_S,pred_T_train,real_T_train               # train()函数返回平均损失、准确率、模型提取到的特征、真实标签，用于后续进一步说明
...
def model_train_with_S(config, dict_source_dataloader, indexed_T_train_loader,T_train_loader,T_test_loader, model,dict_S_cell_cycles,T_test_cell_cycles,T_train_cell_cycles,S_data_dict,S_soh_dict, optimizer,scheduler=None):                # 模型总算法，包含训练及测试
    ...
    if os.path.exists(model_path):                                                              # 如已有参数相同训练模型，则直接进行测试环节，不再进行训练
        ...
        loss_train,train_rmse_loss,train_mmd_loss,  features_train_s,features_train_t, labels_train,dict_pred_S,dict_real_S,pred_T_train,real_T_train = train(config, dict_source_dataloader, indexed_T_train_loader,T_train_loader,T_test_loader, model, optimizer)  # 获取到训练的返回值
        ...
        with trange(config.N_epoch) as t:                                       # 使用trange模块，使输出界面更易读
            for _ in t:
                loss_train,train_rmse_loss,train_mmd_loss,  features_train_s,features_train_t, labels_train,dict_pred_S,dict_real_S,pred_T_train,real_T_train = train(config, dict_source_dataloader, indexed_T_train_loader,T_train_loader,T_test_loader, model, optimizer) # 模型训练
                ...
                if (_+1)%(config.N_epoch//4)==0 or _==0:
                    plt_train_and_test_soh_fig_2(model,S_data_dict,S_soh_dict,dict_S_cell_cycles,T_test_cell_cycles,T_train_cell_cycles,labels_test_pred,labels_test_real,dict_pred_S,dict_real_S,pred_T_train,real_T_train,config,_)
        ...
        plt_tsne(x=combined_features, y=domain_labels, config=config,with_T=True, train=f"domain_fusion_epoch{_ + 1}")
