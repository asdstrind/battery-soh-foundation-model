...
def train(config, dict_source_dataloader, indexed_T_train_loader,T_train_loader,T_test_loader, model, optimizer):  # 模型训练算法
    ...
    train_loss,mmd_loss,rmse_loss ,features_s,features_t, labels_s,features_t_2 =  0,0,0, [],[],[],[]  # 设置基本变量存储部分数据，用于后续的结果输出
    data_s_iter = {
    ...
    for i in range(len_dataloader):
        ...
        feature_fc_t_train,pred_t_train=model(X_t_train)
        features_t_2=np.append(features_t_2,feature_fc_t_train.detach().cpu().numpy())
        ...
        for S_domain_1, S_domain_2 in itertools.combinations(dict_feature_S.keys(), 2):
            ...
            length_mmd+=1
            loss_mmd = mmd+loss_mmd
        loss = (RMSE_loss/(length_rmse^2) + loss_mmd*config.weight_MMD/(length_mmd^2))+RMSE_T_loss*config.weight_rmse_t                                                                   # 计算总损失
        features_t = np.append(features_t, feature_fc_t.detach().cpu().numpy())
        train_loss += loss.detach().cpu().numpy().mean()
        rmse_loss+=(RMSE_loss+RMSE_T_loss).detach().cpu().numpy().mean()
    ...
    return train_loss,rmse_loss,mmd_loss,  features_s.reshape(-1, 256), features_t.reshape(-1, 256),features_t_2.reshape(-1,256), labels_s.reshape(-1, 1),dict_pred_S,dict_real_S,pred_T_train,real_T_train               # train()函数返回平均损失、准确率、模型提取到的特征、真实标签，用于后续进一步说明
...
def model_train_with_S(config, dict_source_dataloader, indexed_T_train_loader,T_train_loader,T_test_loader, model,dict_S_cell_cycles,T_test_cell_cycles,T_train_cell_cycles,S_data_dict,S_soh_dict, optimizer,scheduler=None):                # 模型总算法，包含训练及测试
    ...
    #test_path = os.path.join(config.Result_path, f'test_{config.test_id}', 'with_S_train', 'CNN_LSTM')
    ...
    if os.path.exists(model_path):                                                              # 如已有参数相同训练模型，则直接进行测试环节，不再进行训练
        ...
        with trange(config.N_epoch) as t:                                       # 使用trange模块，使输出界面更易读
            for _ in t:
                loss_train,train_rmse_loss,train_mmd_loss,  features_train_s,features_train_t,features_train_t_2, labels_train,dict_pred_S,dict_real_S,pred_T_train,real_T_train = train(config, dict_source_dataloader, indexed_T_train_loader,T_train_loader,T_test_loader, model, optimizer) # 模型训练
                ...
                if (_+1)%(config.N_epoch//2)==0 or _==0:
                    plt_train_and_test_soh_fig_2(model,S_data_dict,S_soh_dict,dict_S_cell_cycles,T_test_cell_cycles,T_train_cell_cycles,labels_test_pred,labels_test_real,dict_pred_S,dict_real_S,pred_T_train,real_T_train,config,test_path,_)
            ...
            plt_tsne(test_path,x=combined_features, y=domain_labels, config=config,with_T=True, train=f"domain_fusion_epoch{_ + 1}")
            plt_density(test_path,x=combined_features, y=domain_labels, config=config,with_T=True, train=f"{_ + 1}")
            plt_density_2(test_path,x=np.concatenate([features_train_t_2, features_train_t]), y=np.concatenate([np.zeros((len(features_train_t_2), 1)), np.ones((len(features_train_t), 1))]), config=config, train=f"target_{_ + 1}")
        ...
        #pickle.dump(os.path.join(loss_path,'combined_features.pkl'),combined_features)
        #pickle.dump(os.path.join(loss_path,'domain_labels.pkl'),domain_labels)
