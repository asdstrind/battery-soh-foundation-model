...
def generate_dataset(S_data_dict, S_soh_dict, T_data,T_train_data,T_test_data, T_data_soh,T_train_soh,T_test_soh, config):
    dict_S_loader={}
    for domian in S_data_dict.keys():
        ...
        indexed_S_dataset = IndexedTensorDataset(
            torch.FloatTensor(S_x),
            torch.FloatTensor(S_y)
        )
        #S_loader = DataLoader(TensorDataset(S_x, S_y), batch_size=config.Batch_size, shuffle=True,drop_last=False)
        S_loader = DataLoader(indexed_S_dataset, batch_size=config.Batch_size, shuffle=True,drop_last=False)
    ...
    T_loader = DataLoader(TensorDataset(T_x, T_y), batch_size=config.Batch_size, shuffle=False,
                              drop_last=False)
    T_train_x = torch.from_numpy(T_train_data)
    T_train_y = torch.from_numpy(T_train_soh)
    T_train_loader = DataLoader(TensorDataset(T_train_x, T_train_y), batch_size=config.Batch_size, shuffle=True)
    indexed_T_train_dataset = IndexedTensorDataset(
        torch.FloatTensor(T_train_x),
        torch.FloatTensor(T_train_y)
    )
    indexed_T_train_loader=DataLoader(indexed_T_train_dataset, batch_size=config.Batch_size, shuffle=True,drop_last=False)
    T_test_x = torch.from_numpy(T_test_data)
    T_test_y = torch.from_numpy(T_test_soh)
    T_test_loader=DataLoader(TensorDataset(T_test_x, T_test_y), batch_size=config.Batch_size, shuffle=False,drop_last=False)
    indexed_T_test_dataset = IndexedTensorDataset(
        torch.FloatTensor(T_test_x),
        torch.FloatTensor(T_test_y)
    )
    indexed_T_test_loader=DataLoader(indexed_T_test_dataset, batch_size=config.Batch_size, shuffle=False,drop_last=False
    )


    return dict_S_loader,T_loader,indexed_T_train_loader,T_train_loader,T_test_loader,indexed_T_test_loader

def generate_T_train_test_dataset(S_data_dict, S_soh_dict, T_data, T_data_soh, config):
    dict_S_loader = {}
    for domian in S_data_dict.keys():
        S_data = S_data_dict[domian]
        S_data_soh = S_soh_dict[domian]
    ...
    T_x = torch.from_numpy(T_data)
    T_y = torch.from_numpy(T_data_soh)
    T_train_x=T_x[:int(config.train_ratio*len(T_x))]
    T_train_y=T_y[:int(config.train_ratio*len(T_y))]
    T_test_x=T_x[int(config.train_ratio*len(T_x)):]
    T_test_y=T_y[int(config.train_ratio*len(T_y)):]
    T_train_loader = DataLoader(TensorDataset(T_train_x, T_train_y), batch_size=config.Batch_size, shuffle=True,
                                drop_last=False)
    T_test_loader = DataLoader(TensorDataset(T_test_x, T_test_y), batch_size=config.Batch_size, shuffle=False,
                               drop_last=False)
    return dict_S_loader, T_train_loader, T_test_loader
